...
namespace GraphBaseEditor {
    ...
    /// renaming a <see cref="GraphBase.NodeGraph"/> asset, it appears that sometimes the v2 AssetDatabase will swap which asset
    /// is the main asset (present at top level) between the <see cref="GraphBase.NodeGraph"/> and one of its <see cref="GraphBase.Node"/>
    /// sub-assets. As a workaround until Unity fixes this, this asset processor checks all renamed assets and if it
    /// finds a case where a <see cref="GraphBase.Node"/> has been made the main asset it will swap it back to being a sub-asset
