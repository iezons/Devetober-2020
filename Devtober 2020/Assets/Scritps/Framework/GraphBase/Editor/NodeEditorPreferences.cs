...
namespace GraphBaseEditor {
    ...
    public static class NodeEditorPreferences {
        ...
        private static GraphBaseEditor.NodeGraphEditor lastEditor;
        /// <summary> The last key we checked. This should be the one we modify </summary>
        private static string lastKey = "GraphBase.Settings";
        ...
        public static Settings GetSettings() {
            if (GraphBaseEditor.NodeEditorWindow.current == null) return new Settings();

            if (lastEditor != GraphBaseEditor.NodeEditorWindow.current.graphEditor) {
                object[] attribs = GraphBaseEditor.NodeEditorWindow.current.graphEditor.GetType().GetCustomAttributes(typeof(GraphBaseEditor.NodeGraphEditor.CustomNodeGraphEditorAttribute), true);
                if (attribs.Length == 1) {
                    GraphBaseEditor.NodeGraphEditor.CustomNodeGraphEditorAttribute attrib = attribs[0] as GraphBaseEditor.NodeGraphEditor.CustomNodeGraphEditorAttribute;
                    lastEditor = GraphBaseEditor.NodeEditorWindow.current.graphEditor;
        ...
        [SettingsProvider]
        public static SettingsProvider CreateGraphBaseSettingsProvider() {
            SettingsProvider provider = new SettingsProvider("Preferences/Node Editor", SettingsScope.User) {
                guiHandler = (searchContext) => { GraphBaseEditor.NodeEditorPreferences.PreferencesGUI(); },
                keywords = new HashSet<string>(new [] { "GraphBase", "node", "editor", "graph", "connections", "noodles", "ports" })
            };
            return provider;
        }
#endif
        ...
        private static void PreferencesGUI() {
            ...
            if (GUILayout.Button(new GUIContent("Documentation", "https://github.com/Siccity/GraphBase/wiki"), GUILayout.Width(100))) Application.OpenURL("https://github.com/Siccity/GraphBase/wiki");
