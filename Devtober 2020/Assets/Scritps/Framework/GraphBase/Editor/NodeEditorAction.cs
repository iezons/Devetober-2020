...
using UnityEditor;
using UnityEngine;
using GraphBaseEditor.Internal;
...
namespace GraphBaseEditor {
    public partial class NodeEditorWindow {
        ...
        public static GraphBase.Node[] copyBuffer = null;
        ...
        private GraphBase.Node hoveredNode = null;
        [NonSerialized] public GraphBase.NodePort hoveredPort = null;
        [NonSerialized] private GraphBase.NodePort draggedOutput = null;
        [NonSerialized] private GraphBase.NodePort draggedOutputTarget = null;
        [NonSerialized] private GraphBase.NodePort autoConnectOutput = null;
            ...
            switch (e.type) {
                    ...
                    if (e.button == 0) {
                        ...
                        if (currentActivity == NodeActivity.DragNode) {
                            ...
                            for (int i = 0; i < Selection.objects.Length; i++) {
                                if (Selection.objects[i] is GraphBase.Node) {
                                    GraphBase.Node node = Selection.objects[i] as GraphBase.Node;
                                    ...
                                    if (offset.sqrMagnitude > 0) {
                                        foreach (GraphBase.NodePort output in node.Outputs) {
                                            Rect rect;
                                        ...
                                        foreach (GraphBase.NodePort input in node.Inputs) {
                                            Rect rect;
                        ...
                        if (IsHoveringPort) {
                            if (hoveredPort.IsOutput) {
                                ...
                                if (hoveredPort.IsConnected) {
                                    GraphBase.Node node = hoveredPort.node;
                                    GraphBase.NodePort output = hoveredPort.Connection;
                    ...
                    if (e.button == 0) {
                        ...
                        if (IsDraggingPort) {
                            ...
                            if (draggedOutputTarget != null && draggedOutput.CanConnectTo(draggedOutputTarget)) {
                                GraphBase.Node node = draggedOutputTarget.node;
                            ...
                            IEnumerable<GraphBase.Node> nodes = Selection.objects.Where(x => x is GraphBase.Node).Select(x => x as GraphBase.Node);
                            foreach (GraphBase.Node node in nodes) EditorUtility.SetDirty(node);
                            if (NodeEditorPreferences.GetSettings().autoSave) AssetDatabase.SaveAssets();
                        } else if (!IsHoveringNode) {
                ...
                case EventType.KeyDown:
                    ...
                    if (e.keyCode == KeyCode.A) {
                        if (Selection.objects.Any(x => graph.nodes.Contains(x as GraphBase.Node))) {
                            foreach (GraphBase.Node node in graph.nodes) {
                                DeselectNode(node);
                            }
                        } else {
                            foreach (GraphBase.Node node in graph.nodes) {
        ...
        public void Home() {
            var nodes = Selection.objects.Where(o => o is GraphBase.Node).Cast<GraphBase.Node>().ToList();
            if (nodes.Count > 0) {
        ...
        public void RemoveSelectedNodes() {
            ...
            foreach (UnityEngine.Object item in Selection.objects) {
                if (item is GraphBase.Node) {
                    GraphBase.Node node = item as GraphBase.Node;
        ...
        public void RenameSelectedNode() {
            if (Selection.objects.Length == 1 && Selection.activeObject is GraphBase.Node) {
                GraphBase.Node node = Selection.activeObject as GraphBase.Node;
                Vector2 size;
        ...
        public void MoveNodeToTop(GraphBase.Node node) {
            int index;
        ...
        public void DuplicateSelectedNodes() {
            ...
            GraphBase.Node[] selectedNodes = Selection.objects.Select(x => x as GraphBase.Node).Where(x => x != null && x.graph == graph).ToArray();
        ...
        public void CopySelectedNodes() {
            copyBuffer = Selection.objects.Select(x => x as GraphBase.Node).Where(x => x != null && x.graph == graph).ToArray();
        }
        ...
        private void InsertDuplicateNodes(GraphBase.Node[] nodes, Vector2 topLeft) {
            ...
            Dictionary<GraphBase.Node, GraphBase.Node> substitutes = new Dictionary<GraphBase.Node, GraphBase.Node>();
            for (int i = 0; i < nodes.Length; i++) {
                GraphBase.Node srcNode = nodes[i];
                if (srcNode == null) continue;

                // Check if user is allowed to add more of given node type
                GraphBase.Node.DisallowMultipleNodesAttribute disallowAttrib;
                ...
                GraphBase.Node newNode = graphEditor.CopyNode(srcNode);
            ...
            for (int i = 0; i < nodes.Length; i++) {
                GraphBase.Node srcNode = nodes[i];
                if (srcNode == null) continue;
                foreach (GraphBase.NodePort port in srcNode.Ports) {
                    for (int c = 0; c < port.ConnectionCount; c++) {
                        GraphBase.NodePort inputPort = port.direction == GraphBase.NodePort.IO.Input ? port : port.GetConnection(c);
                        GraphBase.NodePort outputPort = port.direction == GraphBase.NodePort.IO.Output ? port : port.GetConnection(c);

                        GraphBase.Node newNodeIn, newNodeOut;
        ...
        bool IsHoveringTitle(GraphBase.Node node) {
        ...
        public void AutoConnect(GraphBase.Node node) {
            if (autoConnectOutput == null) return;

            // Find input port of same type
            GraphBase.NodePort inputPort = node.Ports.FirstOrDefault(x => x.IsInput && x.ValueType == autoConnectOutput.ValueType);
