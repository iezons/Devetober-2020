...
namespace GraphBaseEditor {
    ...
    class NodeEditorAssetModProcessor : UnityEditor.AssetModificationProcessor {
        ...
        private static AssetDeleteResult OnWillDeleteAsset (string path, RemoveAssetOptions options) {
            ...
            if (scriptType == null || (scriptType != typeof (GraphBase.Node) && !scriptType.IsSubclassOf (typeof (GraphBase.Node)))) return AssetDeleteResult.DidNotDelete;
            ...
            for (int i = 0; i < guids.Length; i++) {
                ...
                for (int k = 0; k < objs.Length; k++) {
                    GraphBase.Node node = objs[k] as GraphBase.Node;
        ...
        private static void OnReloadEditor () {
            ...
            string[] guids = AssetDatabase.FindAssets ("t:" + typeof (GraphBase.NodeGraph));
            for (int i = 0; i < guids.Length; i++) {
                string assetpath = AssetDatabase.GUIDToAssetPath (guids[i]);
                GraphBase.NodeGraph graph = AssetDatabase.LoadAssetAtPath (assetpath, typeof (GraphBase.NodeGraph)) as GraphBase.NodeGraph;
                ...
                for (int u = 0; u < objs.Length; u++) {
                    ...
                    if (!graph.nodes.Contains (objs[u] as GraphBase.Node)) graph.nodes.Add(objs[u] as GraphBase.Node);
                }
            }
        }
    }
