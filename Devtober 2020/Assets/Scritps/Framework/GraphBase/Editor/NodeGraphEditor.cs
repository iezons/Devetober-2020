...
namespace GraphBaseEditor {
    ...
    [CustomNodeGraphEditor(typeof(GraphBase.NodeGraph))]
    public class NodeGraphEditor : GraphBaseEditor.Internal.NodeEditorBase<NodeGraphEditor, NodeGraphEditor.CustomNodeGraphEditorAttribute, GraphBase.NodeGraph> {
        ...
        public virtual string GetNodeMenuName(Type type) {
            ...
            GraphBase.Node.CreateNodeMenuAttribute attrib;
        ...
        public virtual void AddContextMenuItems(GenericMenu menu) {
            ...
            for (int i = 0; i < nodeTypes.Length; i++) {
                ...
                if (disallowed) menu.AddItem(new GUIContent(path), false, null);
                    ...
                    GraphBase.Node node = CreateNode(type, pos);
        ...
        public virtual Gradient GetNoodleGradient(GraphBase.NodePort output, GraphBase.NodePort input) {
        ...
        /// <param name="output"> The output this noodle comes from. Never null. </param>
        /// <param name="input"> The output this noodle comes from. Can be null if we are dragging the noodle. </param>
        public virtual float GetNoodleThickness(GraphBase.NodePort output, GraphBase.NodePort input) {
            return 5f;
        }

        public virtual NoodlePath GetNoodlePath(GraphBase.NodePort output, GraphBase.NodePort input) {
            return NodeEditorPreferences.GetSettings().noodlePath;
        }

        public virtual NoodleStroke GetNoodleStroke(GraphBase.NodePort output, GraphBase.NodePort input) {
            return NodeEditorPreferences.GetSettings().noodleStroke;
        }

        /// <summary> Returned color is used to color ports </summary>
        public virtual Color GetPortColor(GraphBase.NodePort port) {
        ...
        public virtual string GetPortTooltip(GraphBase.NodePort port) {
        ...
        public virtual GraphBase.Node CreateNode(Type type, Vector2 position) {
            Undo.RecordObject(target, "Create Node");
            GraphBase.Node node = target.AddNode(type);
        ...
        public virtual GraphBase.Node CopyNode(GraphBase.Node original) {
            Undo.RecordObject(target, "Duplicate Node");
            GraphBase.Node node = target.CopyNode(original);
        ...
        public virtual bool CanRemove(GraphBase.Node node) {
            // Check graph attributes to see if this node is required
            Type graphType = target.GetType();
            GraphBase.NodeGraph.RequireNodeAttribute[] attribs = Array.ConvertAll(
                graphType.GetCustomAttributes(typeof(GraphBase.NodeGraph.RequireNodeAttribute), true), x => x as GraphBase.NodeGraph.RequireNodeAttribute);
        ...
        public virtual void RemoveNode(GraphBase.Node node) {
        ...
        GraphBaseEditor.Internal.NodeEditorBase<NodeGraphEditor, NodeGraphEditor.CustomNodeGraphEditorAttribute, GraphBase.NodeGraph>.INodeEditorAttrib {
            ...
            public CustomNodeGraphEditorAttribute(Type inspectedType, string editorPrefsKey = "GraphBase.Settings") {
