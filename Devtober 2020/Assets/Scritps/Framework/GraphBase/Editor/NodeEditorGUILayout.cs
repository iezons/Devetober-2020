...
namespace GraphBaseEditor {
    /// <summary> GraphBase-specific version of <see cref="EditorGUILayout"/> </summary>
    public static class NodeEditorGUILayout {
        ...
        public static void PropertyField(SerializedProperty property, GUIContent label, bool includeChildren = true, params GUILayoutOption[] options) {
            ...
            GraphBase.Node node = property.serializedObject.targetObject as GraphBase.Node;
            GraphBase.NodePort port = node.GetPort(property.name);
            PropertyField(property, label, port, includeChildren);
        }

        /// <summary> Make a field for a serialized property. Manual node port override. </summary>
        public static void PropertyField(SerializedProperty property, GraphBase.NodePort port, bool includeChildren = true, params GUILayoutOption[] options) {
            PropertyField(property, null, port, includeChildren, options);
        }

        /// <summary> Make a field for a serialized property. Manual node port override. </summary>
        public static void PropertyField(SerializedProperty property, GUIContent label, GraphBase.NodePort port, bool includeChildren = true, params GUILayoutOption[] options) {
            if (property == null) throw new NullReferenceException();
            ...
            if (port == null) EditorGUILayout.PropertyField(property, label, includeChildren, GUILayout.MinWidth(30));
                ...
                if (port.direction == GraphBase.NodePort.IO.Input) {
                    // Get data from [Input] attribute
                    GraphBase.Node.ShowBackingValue showBacking = GraphBase.Node.ShowBackingValue.Unconnected;
                    GraphBase.Node.InputAttribute inputAttribute;
                    ...
                    bool usePropertyAttributes = dynamicPortList ||
                        showBacking == GraphBase.Node.ShowBackingValue.Never ||
                        (showBacking == GraphBase.Node.ShowBackingValue.Unconnected && port.IsConnected);
                    ...
                    if (dynamicPortList) {
                        ...
                        GraphBase.Node.ConnectionType connectionType = inputAttribute != null ? inputAttribute.connectionType : GraphBase.Node.ConnectionType.Multiple;
                        DynamicPortList(property.name, type, property.serializedObject, port.direction, connectionType);
                        return;
                    }
                    switch (showBacking) {
                        case GraphBase.Node.ShowBackingValue.Unconnected:
                            ...
                            break;
                        case GraphBase.Node.ShowBackingValue.Never:
                            // Display a label
                            EditorGUILayout.LabelField(label != null ? label : new GUIContent(property.displayName));
                            break;
                        case GraphBase.Node.ShowBackingValue.Always:
                ...
                } else if (port.direction == GraphBase.NodePort.IO.Output) {
                    // Get data from [Output] attribute
                    GraphBase.Node.ShowBackingValue showBacking = GraphBase.Node.ShowBackingValue.Unconnected;
                    GraphBase.Node.OutputAttribute outputAttribute;
                    bool dynamicPortList = false;
                    ...
                    if (dynamicPortList) {
                        Type type = GetType(property);
                        GraphBase.Node.ConnectionType connectionType = outputAttribute != null ? outputAttribute.connectionType : GraphBase.Node.ConnectionType.Multiple;
                        DynamicPortList(property.name, type, property.serializedObject, port.direction, connectionType);
                        return;
                    }
                    switch (showBacking) {
        ...
        public static void PortField(GraphBase.NodePort port, params GUILayoutOption[] options) {
            PortField(null, port, options);
        }

        /// <summary> Make a simple port field. </summary>
        public static void PortField(GUIContent label, GraphBase.NodePort port, params GUILayoutOption[] options) {
            if (port == null) return;
        ...
        /// <summary> Make a simple port field. </summary>
        public static void PortField(Vector2 position, GraphBase.NodePort port) {
            if (port == null) return;
        ...
        public static void AddPortField(GraphBase.NodePort port) {
            if (port == null) return;
            Rect rect = new Rect();
        ...
        public static void PortPair(GraphBase.NodePort input, GraphBase.NodePort output) {
...
#region Obsolete
        ...
        public static bool IsInstancePortListPort(GraphBase.NodePort port) {
            return IsDynamicPortListPort(port);
        }

        [Obsolete("Use DynamicPortList instead")]
        public static void InstancePortList(string fieldName, Type type, SerializedObject serializedObject, GraphBase.NodePort.IO io, GraphBase.Node.ConnectionType connectionType = GraphBase.Node.ConnectionType.Multiple, GraphBase.Node.TypeConstraint typeConstraint = GraphBase.Node.TypeConstraint.None, Action<ReorderableList> onCreation = null) {
        ...
        public static bool IsDynamicPortListPort(GraphBase.NodePort port) {
        ...
        public static void DynamicPortList(string fieldName, Type type, SerializedObject serializedObject, GraphBase.NodePort.IO io, GraphBase.Node.ConnectionType connectionType = GraphBase.Node.ConnectionType.Multiple, GraphBase.Node.TypeConstraint typeConstraint = GraphBase.Node.TypeConstraint.None, Action<ReorderableList> onCreation = null) {
            GraphBase.Node node = serializedObject.targetObject as GraphBase.Node;
            ...
            var indexedPorts = node.DynamicPorts.Select(x => {
                ...
                return new { index = -1, port = (GraphBase.NodePort) null };
            }).Where(x => x.port != null);
            List<GraphBase.NodePort> dynamicPorts = indexedPorts.OrderBy(x => x.index).Select(x => x.port).ToList();
        ...
        private static ReorderableList CreateReorderableList(string fieldName, List<GraphBase.NodePort> dynamicPorts, SerializedProperty arrayData, Type type, SerializedObject serializedObject, GraphBase.NodePort.IO io, GraphBase.Node.ConnectionType connectionType, GraphBase.Node.TypeConstraint typeConstraint, Action<ReorderableList> onCreation) {
            ...
            list.drawElementCallback =
                (Rect rect, int index, bool isActive, bool isFocused) => {
                    GraphBase.NodePort port = node.GetPort(fieldName + " " + index);
                ...
                (ReorderableList rl) => {
                    ...
                    if (rl.index > reorderableListIndex) {
                        for (int i = reorderableListIndex; i < rl.index; ++i) {
                            GraphBase.NodePort port = node.GetPort(fieldName + " " + i);
                            GraphBase.NodePort nextPort = node.GetPort(fieldName + " " + (i + 1));
            ...
            list.onAddCallback =
                (ReorderableList rl) => {
                    ...
                    if (io == GraphBase.NodePort.IO.Output) node.AddDynamicOutput(type, connectionType, GraphBase.Node.TypeConstraint.None, newName);
            ...
            list.onRemoveCallback =
                (ReorderableList rl) => {
                    ...
                    if (dynamicPorts[index] == null) {
                        ...
                        for (int k = index + 1; k < dynamicPorts.Count(); k++) {
                            for (int j = 0; j < dynamicPorts[k].ConnectionCount; j++) {
                                GraphBase.NodePort other = dynamicPorts[k].GetConnection(j);
                    ...
                    if (hasArrayData && arrayData.propertyType != SerializedPropertyType.String) {
                        ...
                        // Error handling. If the following happens too often, file a bug report at https://github.com/Siccity/GraphBase/issues
            ...
            if (hasArrayData) {
                ...
                while (dynamicPortCount < arrayData.arraySize) {
                    ...
                    if (io == GraphBase.NodePort.IO.Output) node.AddDynamicOutput(type, connectionType, typeConstraint, newName);
                    else node.AddDynamicInput(type, connectionType, typeConstraint, newName);
                    EditorUtility.SetDirty(node);
                    dynamicPortCount++;
                }
