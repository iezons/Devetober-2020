...
namespace GraphBaseEditor {
    ...
    public partial class NodeEditorWindow {
        ...
        private List<GraphBase.Node> culledNodes;
        ...
        void ShowPortContextMenu(GraphBase.NodePort hoveredPort) {
            GenericMenu contextMenu = new GenericMenu();
            ...
            foreach (GraphBase.Node node in graph.nodes) {
                ...
                foreach (GraphBase.NodePort output in node.Outputs) {
                    ...
                    for (int k = 0; k < output.ConnectionCount; k++) {
                        GraphBase.NodePort input = output.GetConnection(k);
        ...
        private void DrawNodes() {
            ...
            if (Selection.activeObject != null && Selection.activeObject is GraphBase.Node) {
            ...
            List<GraphBase.NodePort> removeEntries = new List<GraphBase.NodePort>();

            if (e.type == EventType.Layout) culledNodes = new List<GraphBase.Node>();
            for (int n = 0; n < graph.nodes.Count; n++) {
                // Skip null nodes. The user could be in the process of renaming scripts, so removing them at this point is not advisable.
                if (graph.nodes[n] == null) continue;
                if (n >= graph.nodes.Count) return;
                GraphBase.Node node = graph.nodes[n];
        ...
        private bool ShouldBeCulled(GraphBase.Node node) {
